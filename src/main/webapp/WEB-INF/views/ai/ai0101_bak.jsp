...
<body>
						...
						<div class="back-box" style="height: 98%;">
							<form id="atchForm" class="main-contents-grid1"
								...
								<input type="hidden" name="evnt_id" /> <input type="hidden"
									name="atch_file_id" />
								<div class="main-contents-grid2"
									style="column-gap: 2rem; flex-wrap: wrap;">
									<div class="main-contents-grid1">
										<h2>이벤트 분류</h2>
										<div id="evntSeBtnBox"
											style="display: flex; column-gap: 0.65rem;">
								...
								<div class="main-contents-grid1">
									<div class="main-contents-grid2">
										...
										<img class="slide-down" id="btnSlideSetting"
											onclick="toggleSettingBox(this);">
										<p id="addSettingTxt"></p>
									</div>
									<div id="settingBox"
								...
								<div class="main-contents-grid1">
									<h2>시작/종료 일시</h2>
									<div class="main-contents-grid2" style="flex-wrap: wrap;">
									...
									<div id="titleEvntAdd1" class="main-contents-grid2"
										...
										<h2 style="display: inline-flex; align-items: center;">이벤트
											설명</h2>
										<div class="main-contents-grid2">
											<button class="add" onclick="showAddEvntForm('add')">
												<img />
											</button>
											<button class="save" onclick="onSaveEvntInfo();"></button>
										</div>
									</div>
									<div id="titleEvntModify1" class="main-contents-grid2"
										style="display: none; justify-content: space-between;">
										<h2 id="modyTitle">이벤트 설명</h2>
										<div class="main-contents-grid2">
											<button class="add" onclick="showAddEvntForm('add')">
												<img />
											</button>
											<button class="save" onclick="onUpdateEvntInfo();"></button>
	...
	<script>
				...
				rowClick: function(args) {
					...
					if (args.event.target.className != "evnt-atch-file-icon") {
						...
						setTimeout(function() {
							...
							CKEDITOR.instances.evnt_cn.insertHtml(rowData.evnt_cn);
						}, 1);
		...
		async function init() {
			...
			document.querySelectorAll(".btn-evnt").forEach((v, i) => {
				v.addEventListener("click", function(e){
					...
					toggleSettingBox(btnSlideSetting, true);
				});
			});
		...
		function initAddEvntForm() {
			...
			setDateTimeNoLimit($("#strtDate"), $("#strtTime"), start);
			setDateTimeNoLimit($("#endDate"), $("#endTime"), end);
		...
		function onSaveEvntInfo() {
			...
			form.bgng_dt.value = ($("#strtDate").val()).replaceAll("-", "") + ($("#strtTime").val()).replaceAll(":", "");
			if (form.bgng_dt.value.length == 12) {
				form.bgng_dt.value += "00";
			}
			form.end_dt.value = ($("#endDate").val()).replaceAll("-", "") + ($("#endTime").val()).replaceAll(":", "");
		...
		function onUpdateEvntInfo() {
			...
			let bgng_dt = $("#strtDate").val() + " " + $("#strtTime").val();
			...
			if (!!fclt_id) {
				...
				document.getElementById("BTNGRP_FD").querySelectorAll(".btn-tag").forEach(button => {
					button.addEventListener("click", function() {
						...
						AddSettingText();
						toggleSettingBox(btnSlideSetting, false);
		...
		function toggleSettingBox(e, isShow) {
			if (isShow == null || isShow == undefined) {
				if (e.classList.value == "slide-down") {
					isShow = true;
				} else {
					isShow = false;
				}
