...
(function(window, $, undefined) {
	...
	function addTags(workspace, tagList) {
		for(let tagName of tagList) {
			workspace.tagList[tagName] = tagName;
		}
	}
	...
	function workspace(element, config) {
		...
		this.ckIcon.src = this.imagesPath + "/biz/check_on.png"
	...
	workspace.prototype = {
		...
		onTimer : function() {
			...
			if(now > this.request) {
				...
				this.request = new Date(this.request.setSeconds(this.request.getSeconds() + this.interval / 1000));
			}
			*/
...
//			this.refresh();
		...
		setSize : function(width, height) {
			...
			if(typeof(this.backCanvas) != 'undefined') {
				...
				this.backCanvas.style.top = -height;
			}
			...
			for(let shape of uxList.shapes) {
				...
				if(typeof(val) != 'undefined') {
                    if(typeof(val.val) != 'undefined') {
                        let strVal = NumbericFormat(val.val, shape.effectByValue.decPos, shape.effectByValue.commaDelimeted);
		...
		this.each(function() {
			...
			if(typeof(instance) == 'undefined') {
				...
				switch(config) {
					...
					case "clearSelection" : clearSelection(c); break;
...
//(function(bizRender, $, undefined) {
	...
	uxBase.prototype = {
		...
		fillText : function(context) {
			if(typeof(this.fill.text) != 'undefined') {
				...
				let metrics = context.measureText(this.fill.text);
				...
				context.fillText(this.fill.text, x, y);
		...
		recalcRect : function(zoomRatio) {
			this.left = zoomRatio * this.x;
			this.top = zoomRatio * this.y;
			this.width = zoomRatio * this.cx;
			this.height = zoomRatio * this.cy;
	...
	clearBackground = function(context, uxList, zoom) {
		context.clearRect(0, 0, uxList.width * zoom, uxList.height * zoom);
	}
	...
	recalcRect = function(uxList, zoomRatio) {
		for(let shape of uxList.shapes) {
			if(shape.bDraw == false) continue;
			shape.recalcRect(zoomRatio); //shape.recalcRect(shape, zoomRatio);
		}
	}
	...
	testEffectByImagelist = function(workspace, shape) {
		if(typeof(shape.effectByImagelist) != 'undefined') {
			...
			let tag = workspace.tagList[shape.effectByImagelist.tagName];
	...
	drawUxList = function(workspace, zoomRatio, tick) {
		...
		for(let shape of uxList.shapes) {
			...
			if(typeof(shape.fill) != 'undefined') {
				if(shape.type != "line") {
					context.font = shape.font.fontStyle + " " + (shape.font.fontSize * zoomRatio).toString() + "px " + shape.font.fontFamily;
                    shape.fillBack(context, zoomRatio);
			...
			if(shape.select == true) {
				...
				context.drawImage(workspace.ckIcon, shape.left, shape.top + 10, 15, 15);
			}

			shape.recalcRect(zoomRatio);

			shape.clearRect.left = shape.border.x * zoomRatio - 2 * Math.max(shape.line.width, 1);
			shape.clearRect.top = shape.border.y * zoomRatio - 2 * Math.max(shape.line.width, 1);;
			shape.clearRect.width = shape.border.cx *  zoomRatio + 4 * Math.max(shape.line.width, 1);
			shape.clearRect.height = shape.border.cy * zoomRatio + 4 * Math.max(shape.line.width, 1 );
	...
	render.prototype = {
		calcZoomRate : function(workspace) {
			...
			return Math.min(zoomRatioX, zoomRatioY);
		},
		...
		drawBack : function(workspace) {
			...
			if(typeof(workspace.uxList.backImage) != 'undefined') {
				...
				context.fillRect(0, 0, workspace.uxList.width * zoomRatio, workspace.uxList.height * zoomRatio);
			}
				...
				if(xmlHttp.readyState == 4) {
					if(xmlHttp.status  = 200) {
						...
						if(xmlHttp.responseXML != null) {
							...
							location.href = "building-manager";
						}
					}
				}
			}
